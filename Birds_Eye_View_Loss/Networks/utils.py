...
def define_args():
    ...
    parser.add_argument('--nepochs', type=int, default=450, help='total numbers of epochs')
...
def draw_homography_points(img, x, resize=256, color=(255,0,0)):
    ...
    [cv2.circle(np.asarray(img.copy()), tuple(idx), radius=5, thickness=-1, color=(255,0,0)) for idx in src]
    [cv2.circle(np.asarray(img.copy()), tuple(idx), radius=5, thickness=-1, color=(0,255,0)) for idx in dst_ideal]
    [cv2.circle(np.asarray(img.copy()), tuple(idx), radius=5, thickness=-1, color=(0,0,255)) for idx in dst]
    return img
