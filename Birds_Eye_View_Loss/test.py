...
def main():
    ...
    model = Net(args)
    checkpoint = torch.load("./Saved/Mod_erfnet_opt_adam_loss_area_lr_0.0001_batch_8_end2end_True_lanes_2_resize_256_pretrainFalse_clasFalse/checkpoint_model_epoch_350.pth.tar")
    model.load_state_dict(checkpoint['state_dict'])

    #device = torch.device('cuda')
    #model = Net(args)
    #model = torch.load("Saved/Mod_erfnet_opt_adam_loss_area_lr_0.0001_batch_8_end2end_True_lanes_2_resize_256_pretrainFalse_clasFalse/model_best_epoch_319.pth.tar")
    model = model.cuda()
